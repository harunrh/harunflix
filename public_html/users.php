...
<div class="content-row">
    ...
    <!-- Mobile Users List (displays on smaller screens) -->
    <div class="d-md-none">
                    ...
                    <a href="profile.php?id=<?php echo $user['user_id']; ?>" class="list-group-item list-group-item-action">
                        <div class="d-flex align-items-center">
                            <?php if ($user['profile_picture']): ?>
                                <img src="<?php echo $user['profile_picture']; ?>" alt="<?php echo $user['username']; ?>" class="rounded-circle me-3" style="width: 40px; height: 40px; object-fit: cover;">
                            <?php else: ?>
                                <div class="avatar rounded-circle text-white d-flex align-items-center justify-content-center me-3" style="width: 40px; height: 40px; font-size: 18px;">
                                    <?php echo strtoupper(substr($user['username'], 0, 1)); ?>
                                </div>
                            <?php endif; ?>
                            <div class="flex-grow-1">
                                <div class="d-flex justify-content-between align-items-center">
                                    ...
                                    <span class="badge bg-primary rounded-pill"><?php echo $user['review_count']; ?></span>
                                </div>
                                <div class="small text-muted">
                                    Joined <?php echo date('n/j/y', strtotime($user['created_at'])); ?>
                                </div>
                            </div>
                        </div>
                    </a>
    ...
    <!-- Desktop Users Table (hidden on smaller screens) -->
                            ...
                            <?php foreach ($users as $user): ?>
                                <tr data-user-id="<?php echo $user['user_id']; ?>">
                                    ...
                                    <td>
                                        <?php if ($user['avg_rating']): ?>
                                            <div class="d-flex align-items-center">
                                                <span class="me-2"><?php echo number_format($user['avg_rating'], 1); ?></span>
...
$(document).ready(function() {
    ...
    $("#user-search").on("keyup", function() {
        ...
        $("#users-table tbody tr, .list-group a").filter(function() {
            $(this).toggle($(this).text().toLowerCase().indexOf(value) > -1);
        });
    });
    
    // Make desktop rows clickable
